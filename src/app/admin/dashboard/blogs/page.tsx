...
export default function BlogManagementPage() {
  ...
  const fetchBlogPosts = async () => {
    try {
      ...
      const response = await adminApi.get(`/api/admin/blogs?${params}`)
      
      if (response.success) {
        // Transform backend data to match frontend interface
        const transformedPosts = response.data.map((post: any) => ({
          ...post,
          category: post.categories?.[0] || { name: 'Uncategorized', slug: 'uncategorized', color: '#6B7280' },
        ...
        showToast({
          type: 'error',
          title: 'Error',
          message: response.error || 'Failed to fetch blog posts'
        })
      }
    } catch (error) {
  ...
  const fetchBlogOverview = async () => {
    try {
      const response = await adminApi.get('/api/admin/blogs/overview')
      ...
      if (response.success) {
        ...
        showToast({
          type: 'error',
          title: 'Error',
          message: response.error || 'Failed to fetch blog overview'
        })
      }
    } catch (error) {
  ...
  // Use posts directly from API (already filtered and paginated)
